...
WORKDIR /src
COPY ["Dream.WebApi/Dream.WebApi.csproj", "Dream.WebApi/"]
RUN dotnet restore "Dream.WebApi/Dream.WebApi.csproj"
COPY . .
WORKDIR "/src/Dream.WebApi"
RUN dotnet build "Dream.WebApi.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Dream.WebApi.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Dream.WebApi.dll"]